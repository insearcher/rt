...
int		get_directional_lights(t_light *lights, int *cur_light, t_jnode *n)
{
	t_jnode	*tmp;
	t_jnode	*tmp2;
	int		i;
...
int		get_point_lights(t_light *lights, int *cur_light, t_jnode *n)
{
	t_jnode	*tmp;
	t_jnode	*tmp2;
	int		i;
...
int	rt_jtoc_get_lights(t_scene *scene, t_jnode *n)
{
	...
	if (scene->lights_count != 0)
	{
		...
		cur_light = 0;
		if (get_directional_lights(lights, &cur_light, jtoc_node_get_by_path(n, "directional_lights")))
			return (rt_jtoc_sdl_log_error("DIRECTIONAL LIGHTS ERROR", -1));
		if (get_point_lights(lights, &cur_light, jtoc_node_get_by_path(n, "point_lights")))
