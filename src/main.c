...
/*   Updated: 2019/12/01 19:09:50 by sbednar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			setup_systems(t_rt_main *rt, t_ui_main *ui)
{
	...
	ps = ft_memalloc(sizeof(t_physics_system));
	...
	is = ft_memalloc(sizeof(t_input_system));
	is->system.parent = is;
	is->state = ui->state;
	is->active = vec_at(&ps->rbs, 0);
	system_setup(&is->system, "input", is_func, 3);
	system_start(&is->system);
	rt->systems = vec_init(1, sizeof(t_system *));
	vec_push_back(rt->systems, &(is->system));
	vec_push_back(rt->systems, &(ps->system));
}
...
int					main(int argc, char **argv)
{
	...
	if (argc == 1)
		return (-1);
	if (argc > 1)
		rt = rt_setup(rt_screen_size, "json/textures.json", argv[1]);
	ui->data = (void *)rt;
