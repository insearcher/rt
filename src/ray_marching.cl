...
static t_vector3d mod(t_vector3d a, t_vector3d b)
{
	int k = (sqrt(a.x * a.x + a.y * a.y + a.z * a.z) / sqrt(b.x * b.x + b.y * b.y + b.z * b.z));
	return ((t_vector3d){a.x - b.x * k, a.y - b.y * k, a.z - b.z * k});
//	if (a < b)
//		b = a;
//	if (b == 0)
//		return (a);
...
{
//	t_vector3d in = mod(a, b);
//	t_vector3d in = mv_mult(a, b);
//	in = mv_minus(in, mv_mult_num(b, 0.5f));
//	return (mv_length(in) - radius);
	return (mv_length(mv_minus(pos, c)) - radius);
//	return (mv_length(mv_minus((t_vector3d){mod(O.x, obj->center.x),
//									mod(O.y, obj->center.y), mod(O.z, obj->center.z)}, mv_mult_num(obj->center, 0.5) - radius);
}
