...
static void	rotate(t_ui_el *el, t_object *obj, int dir)
{
	if ((el->id / 10) % 10 == 1)
		rotate_transform_around_axis(&obj->transform,
				(cl_float3){{1, 0, 0}}, (float)dir * CHANGE_ROT_SPEED);
	else if ((el->id / 10) % 10 == 2)
		rotate_transform_around_axis(&obj->transform,
				(cl_float3){{0, 1, 0}}, (float)dir * CHANGE_ROT_SPEED);
	else if ((el->id / 10) % 10 == 3)
		rotate_transform_around_axis(&obj->transform,
				(cl_float3){{0, 0, 1}}, (float)dir * CHANGE_ROT_SPEED);
}
