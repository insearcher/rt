...
void	run_render(t_rt_main *rt, t_ui_el *el, cl_mem *mem_img,
		cl_mem *mem_objects)
{
	int		err;
	size_t	global_size;
	...
	err |= clSetKernelArg(*kernel, 5, sizeof(cl_float3), &rt->scenes[0].camera.transform.pos);
	err |= clSetKernelArg(*kernel, 6, sizeof(cl_float3), &rt->scenes[0].camera.transform.local.right);
	err |= clSetKernelArg(*kernel, 7, sizeof(cl_float3), &rt->scenes[0].camera.transform.local.up);
	err |= clSetKernelArg(*kernel, 8, sizeof(cl_float3), &rt->scenes[0].camera.transform.local.forward);
		...
		cl_mem *mem_objects)
{
	...
	*mem_img = clCreateBuffer(*rt->cl->context, CL_MEM_READ_WRITE,
			el->rect.w * el->rect.h * sizeof(int), NULL, &err);
	if (err != 0)
		SDL_Log("create buffer - error\n");
	*mem_objects = clCreateBuffer(*rt->cl->context, CL_MEM_READ_ONLY,
			sizeof(t_object) * rt->scenes[0].objects_count, NULL, &err);
	if (err != 0)
		SDL_Log("create buffer - error\n");
...
int		rt_render(t_ui_main *ui, void *a)
{
	...
	cl_mem		mem_img;
	cl_mem		mem_objects;
	get_mem_for_render(rt, el, &mem_img, &mem_objects);
	run_render(rt, el, &mem_img, &mem_objects);
