...
static float sdf(cl_float3 origin, t_object *obj)
{
	...
	cl_float3 new_local_pos = (cl_float3) {{
												   f3dot((cl_float3) {{
																			  cos + (1 - cos) * a.x * a.x,
																			  (1 - cos) * a.x * a.y - sin * a.z,
																			  (1 - cos) * a.x * a.z + sin * a.y
																	  }}, local_pos),
												   f3dot((cl_float3) {{
																			  (1 - cos) * a.x * a.y + sin * a.z,
																			  cos + (1 - cos) * a.y * a.y,
																			  (1 - cos) * a.y * a.z - sin * a.x
																	  }}, local_pos),
												   f3dot((cl_float3) {{
																			  (1 - cos) * a.x * a.z - sin * a.y,
																			  (1 - cos) * a.y * a.z + sin * a.x,
...
//	local_pos = repeatSDF(local_pos, obj->transform.pos, 0, 0, 0);
	switch (obj->type)
	{
		...
		case o_torus:
			...
			break;
		case o_capped_torus:
			distance = sdf_capped_torus(local_pos, obj->params.capped_torus.sc, obj->params.capped_torus.ra,
										obj->params.capped_torus.rb);
			break;
		case o_link:
